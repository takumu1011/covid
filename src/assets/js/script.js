...
$(function () {
  ...
  var $naviHeight = $('#navi').height() + 20;
  $('.navi_1, #modal_1').on('click', function () {
    $('html,body').animate({
      scrollTop: $('#l-about').offset().top - $naviHeight
    });
  });
  $('.navi_2, #modal_2').on('click', function () {
    $('html,body').animate({
      scrollTop: $('#l-3cs').offset().top - $naviHeight
    });
  });
  $('.navi_3, #modal_3').on('click', function () {
    $('html,body').animate({
      scrollTop: $('#l-wecan').offset().top - $naviHeight
    });
  });
  ...
  var scene = new ScrollMagic.Scene({
    ...
    offset: 0,
    reverse: true
  }).setClassToggle("#navi-fix", "fixed").addTo(controller);
  var scene2 = new ScrollMagic.Scene({
    triggerElement: "#l-3cs",
    triggerHook: 'onCenter',
    offset: 0,
    reverse: false
  }).setClassToggle("#l-3cs", "enter").addTo(controller);
});

function slideJudge(ele) {
  var target = $(ele).data('toggle');
